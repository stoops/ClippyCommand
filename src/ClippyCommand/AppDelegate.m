...
int loopmenu, loopsave, loopflag;
...
- (NSString *)safeText:(NSString *)strs {
    ...
    NSString *outp = [[strs componentsSeparatedByCharactersInSet:invr] componentsJoinedByString:@""];
    return outp;
}
            ...
            if ([pt containsString:@"utf8"]) {
                ...
                if (pz && (pz.length > 0)) {
                    ...
                    if (mode == 0) {
                        ...
                        NSString *rp = [self safeText:pz];
                        if ((1 <= rp.length) && (rp.length <= 999)) {
                            int f = -1;
                            for (int k = 0; (f == -1) && (k < [historyl count]); ++k) {
                                NSString *hp = [historyl objectAtIndex:k];
                                if ([hp isEqualToString:rp]) { f = (k + 1); }
                            }
                            if ((f < 0) || (f > 1)) {
                                int maxl = 15;
                                if (f > 0) { [historyl removeObjectAtIndex:f-1]; }
                                [historyl insertObject:rp atIndex:0];
...
- (void)procMenu {
    long offs = 1;
    long leng = ([commands count] + offs + 1);
    long indx = [menubar.button.menu numberOfItems];
    ...
    while (indx > leng) {
        [menubar.button.menu removeItemAtIndex:(indx-1)];
        indx -= 1;
    }
    ...
    NSMenuItem *subm = [menubar.button.menu itemAtIndex:0];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    loopmenu = 0; loopsave = 0; loopflag = 0;
    [self timeLoop:nil];
