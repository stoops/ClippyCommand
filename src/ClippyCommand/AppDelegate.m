...
NSMutableArray *commands, *buttonsl, *undolist;
...
- (void)threadFun:(NSArray *)object {
    ...
    NSData *stdata = [stdstr dataUsingEncoding:NSUTF8StringEncoding];
    ...
    for (int i = 0; i < [pa count]; ++i) {
        ...
        for (int j = 0; j < [pl count]; ++j) {
            ...
            if ([nstr containsString:@"utf8"]) {
                ...
                NSString *pz = [[NSString alloc] initWithBytes:[ndat bytes] length:[ndat length] encoding:NSUTF8StringEncoding];
...
- (void)procMenu {
    ...
    NSMenuItem *item = [menubar.button.menu itemAtIndex:(leng-1)];
    [item setTag:0]; [item setTitle:@"Undo"];
    [item setAction:@selector(menuUndo:)];
    [item setKeyEquivalent:[NSString stringWithFormat:@"Z"]];
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    NSString *file = [NSString stringWithFormat:@"%@/Contents/Resources/data/clippys.png", path];
    ...
    [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(timeLoop:) userInfo:nil repeats:YES];
}
...
- (void)menuBar:(NSNotification *)aNotification {
    ...
    [menubar.button.menu popUpMenuPositioningItem:nil atLocation:posi inView:nil];
}
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    while (row >= [buttonsl count]) {
        ...
        NSTextField *txtb = [self makeText:NSMakeRect(0, 9, 9999, 22)];
