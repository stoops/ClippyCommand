            ...
            if ([nstr containsString:@"utf8"]) {
                ...
                if (pz && ([pz length] > 0)) {
                    ...
                    if (mode == 0) {
                        ...
                        int f = 0;
                        for (int j = 0; j < [historyl count]; ++j) {
                            NSString *hp = [historyl objectAtIndex:j];
                            if ([hp isEqualToString:pz]) { f = 1; }
                        }
                        if (f == 0) {
                            int maxl = 15;
...
- (void)procMenu {
    ...
    for (int i = 0; i < [historyl count]; ++i) {
        NSString *hp = [historyl objectAtIndex:i];
        NSInteger no = ([[hp componentsSeparatedByString:@"\n"] count] - 1);
        NSString *zp = [NSString stringWithFormat:@"(%d) %@...", (int)no, (hp.length > maxl) ? [hp substringToIndex:maxl] : hp];
        NSMenuItem *item = [[NSMenuItem alloc] init];
        [item setTag:(i+1)]; [item setTitle:zp];
